...
namespace SimpleFramework.Xml.Stream {
   ...
   class OutputBuffer {
      ...
      /// <summary>
      /// This will add a <c>char</c> to the end of the buffer. The
      /// buffer will not overflow with repeated uses of the
      /// <c>Append</c>, it uses an <c>EnsureCapacity</c> method
      ...
      /// <c>Append</c>, it uses an <c>EnsureCapacity</c> method
      /// which will allow the buffer to dynamically grow in
      ...
      /// <summary>
      /// This will add a <c>char</c> array to the buffer. The
      /// buffer will not overflow with repeated uses of the
      /// <c>Append</c>, it uses an <c>EnsureCapacity</c> method
      /// method which will allow the buffer to dynamically grow 
      /// in size to accommodate large character arrays.
      /// </summary>
      /// <param name="value">
      ...
      /// <c>append</c>, it uses an <c>ensureCapacity</c>
      /// method which will allow the buffer to dynamically grow 
      /// in size to accommodate large string objects.
      /// </summary>
      /// <param name="value">
      ...
      /// the specified <c>Writer</c> object. This is used when the
      /// XML element is to be committed to the resulting document.
      /// </summary>
      /// <param name="out">
